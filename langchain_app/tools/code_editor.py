...
class CodeEditorTooling:
    def __init__(self, filename: Optional[str] = None, load_source_code_on_start=False) -> None:
        self.source_code: List[str] = []
        self.filename = filename
        if not self.filename:
            self.filename = [random.choice("abcdefghijklmnoqrstuvxwyz") for r in range(10)]
            self.filename += ".py"
        print("Init code editor with filename: ", self.filename)
        if load_source_code_on_start:
            print("Loading existing source code")
    ...
    def display_code(self):
        code_string = ""
        for _, line in enumerate(self.source_code):
            code_string += f"{line}\n"
        return code_string
