...
from langchain_app.simple_loop.prompts import Identities
...
def strip_hallucination(output, stop_token=None):
    if stop_token is None:
        stop_token = Identities.MASTERMIND
    if stop_token in output:
        return output.split(stop_token)[0]
    return output
